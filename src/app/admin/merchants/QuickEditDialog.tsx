...
export default function QuickEditDialog({ merchant }: { merchant: MerchantRow }) {
  ...
  return (
    <>
      <button className="underline" onClick={() => setOpen(true)} aria-label={`Quick edit ${merchant.name}`}>
        Edit
      </button>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setOpen(false)} className="text-white/60">Close</button>
            </div>
      ...
      {msg && (
        <Toast kind={msg.kind} show onClose={() => setMsg(null)}>
          {msg.text}
        </Toast>
      )}
    </>
