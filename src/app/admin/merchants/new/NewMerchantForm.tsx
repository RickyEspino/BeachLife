...
type Props = {
  // Server action signature from page.tsx:
  // export async function createMerchantAction(_: any, formData: FormData)
  action: (
...
export default function NewMerchantForm({ action }: Props) {
  ...
  const fieldError = useMemo(
    () => (state?.ok ? null : state?.field ?? null),
    [state]
  );
    ...
    <form action={formAction} className="grid gap-3">
      ...
      <div className="grid gap-1">
        ...
        <p className="text-xs text-white/50">
          If the email is already a user, they’ll be linked as{" "}
